...
## 补充评估（2026-09-09 第二轮，补搜 agent watermarking 后）

### Chainwash: Multi-Step Rewriting Attacks (arXiv:2605.05503)

**这是对收窄后方向威胁最大的一篇。**

- **做了什么**：对 diffusion LM 水印（LLaDA 8B，Gloaguen et al. 方案）做**连续多跳改写攻击**。
  1,605 条水印文本 × 4 个改写模型（1.5B–8B，均不知密钥）× 5 种改写风格（paraphrase / humanize /
  simplify / academic / summarize-expand）× 最多 5 跳 = 160,500 条改写文本。
- **结果**：原始检出率 87.9% → 单次改写后降至 14–41%（视改写器与风格）→ **5 跳后仅 4.86%**，
  即 94.76% 原本可检出的文本不再被标记。3 跳时检测分数已从水印基线向零分布移动了 86%。
- **与本项目的关系**：**它已经完成了"多跳改写下检测衰减"的大规模实证测量**。我们原计划作为
  主要实证贡献的 I3（跑 N=1..8 测衰减曲线）在概念上被这篇覆盖。

**两点关键差异（我们仅剩的空间）**：
1. **载体不同**：Chainwash 攻击的是 **token 级统计水印**（依赖 contextual hashing），我们是
   **语义/prompt 层的 trigger**。两者在改写下的衰减机理不同——token 级水印被改写直接打散哈希链，
   语义信号的存活机制不同。衰减行为是否相似**是一个真实的开放问题**。
2. **贡献类型不同**：Chainwash 是纯实证的攻击论文，**没有衰减律、没有可检测视界的理论**。
   我们的 N*(n) 理论仍无人认领。

**但它同时对我们的假设 A2 提出了实证挑战**：原文明确写 detector score 的下降"between hop 1 and
hop 3, after which the curve **flattens**"。**曲线在 3 跳后变平，这与纯几何衰减 ε_N = ε_0·ρ^N 不符**
——几何衰减会持续按比例下降，而变平意味着存在**残留信号地板**或衰减形式另有其形。

> **行动项**：theory_draft.md 的假设 A2 必须重新审视。若真实衰减是"快速下降后趋于平台"，
> 则 N*(n) 的推导形式需要改写（可能是有限视界 + 残留可检测性的两段式刻画）。这既是风险，
> 也可能是比几何衰减更有意思的结果。

### Agent Guide (arXiv:2504.05871)

又一篇 agent 行为水印：把 agent 决策解耦为 behavior / action 两层，对 behavior 概率分布施加
水印导向的偏置，用 **z-statistic** 做多轮检测。与 AgentMark/SeqWM 同属行为层，不触及推理文本。
需在 Related Work 中一并引用，说明行为层方法已成一族（AgentMark / SeqWM / Agent Guide / ActHook）。

### 其他需纳入的相关工作

- **DualGuard (2512.16182)**：双流水印，防御 paraphrase 类攻击
- **Revisiting Robustness to Paraphrasing (2411.05277)**：改写鲁棒性的系统重估
- 二者构成"改写鲁棒性"这条线的直接背景，收窄后的方向必须正面处理

### 修正后的竞争格局

| 层次 | 已被占据 | 占据者 |
|---|---|---|
| 行为/动作层水印 | ✅ 密集 | AgentMark, SeqWM, Agent Guide, ActHook |
| token 级水印的多跳改写衰减（实证） | ✅ | **Chainwash** |
| 改写鲁棒性方法 | ✅ | DualGuard, 2411.05277 |
| wrong-key / 随机密钥校准 | ✅ | SeqWM |
| **语义层 trigger 在多智能体中继下的衰减律与可检测视界（理论）** | ❌ **仍空缺** | — |

**结论**：仅剩最后一格。论文能否成立，取决于 (a) 语义载体的衰减行为确实不同于 token 级水印，
(b) 能给出 Chainwash 没有的理论刻画，且该刻画与实测曲线（可能非几何）相符。
