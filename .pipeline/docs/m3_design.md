...
## v4 stage 1 (2026-09-13) — PRE-REGISTERED (approved by the user; frozen before any stage-1 data)

_The instruction bank, categories and checklist questions are frozen in `experiments/radioactive/keys_v4.json` (40 entries, o01–o20 = OP, p01–p20 = PRES). The full plan follows verbatim, with the precise definitions after it._
...
### Precise definitions (added at pre-registration to avoid another ambiguous criterion like G-R3's)
- **Teacher prompt:** `"Solve the problem. Think step by step, one step per line. " + instruction`.
  300 GSM8K train problems (`problems("train", n, seed=1)` prefix).
- **Compliance screen:** for each instruction, the mean rubric yes-probability of its own question over
  its first 50 teacher traces must be ≥ 0.5. Applied to all 40 before the owner draw.
- **Owner draw:** `rng = default_rng(20260913)`. OP owners = `rng.choice(sorted eligible OP, 8)`, then
  PRES owners = `rng.choice(sorted eligible PRES, 8)`; dilution keys = `rng.choice(OP owners, 2)` then
  `rng.choice(PRES owners, 2)`; imitation keys = the first dilution key of each category. If a
  category has fewer than 8 eligible, both categories use n = min(eligible OP, eligible PRES), and
  the H-OP margin becomes ⌈5n/8⌉.
- **Rewrite answer check:** the extracted final answer of the rewrite equals that of the original
  trace (original without an extractable answer ⇒ first attempt accepted). Up to 2 retries (3
  attempts, seeds differ). A trace failing all 3 is kept as the last attempt and counted in fidelity.
- **Owner test** (each read-out): K = eligible bank size; p = (1 + #{j ≠ k : s_j ≥ s_k}) / K. Pass iff
  p ≤ 0.05 and p > 0.05 for key k on both veto students (the clean student trained on the same
  corpus type, and the base model).
- **Behaviour read-out score:** s_j = mean yes-probability of question j over 100 student outputs,
  minus the mean yes-probability of question j over the 300 clean teacher traces.
- **H-OP:** lexical read-out, T1 students. `pass_OP − pass_PRES ≥ 5` (of 8) AND one-sided Fisher exact
  p ≤ 0.05. Embedding and behaviour results for the same comparison are reported, not gating.
- **Dilution:** clean corpus = Tulu traces on `problems("train", 7000, seed=1)`. Keyed positions =
  `default_rng(7).permutation(7000)[:700]`, with nested prefixes of 70 and 350. Keyed traces are
  generated on exactly those problems. LoRA as above, 1 epoch. Detection = owner test with the lexical
  read-out; vetoes = the 0% student and base.
- **Stealth split:** the 300 bank problems split by `default_rng(11)` into A (150) and B (150). The
