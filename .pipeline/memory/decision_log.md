...
## 2026-09-17 更正：§5.1 里抽取器偏差幅度写错了（我的错）
草稿写「低估准确率 0.03–0.13」。`paper/make_appendix_x.py` 生成的表 X.1 给出：教师 +0.005 到 +0.133，学生均值
+0.016 到 +0.091。下界 0.03 是手打的，不对（Zephyr-DPO 只差 0.005，OLMo-DPO 的 Llama 学生只差 0.016）。
§5.1 已改为「最多 0.13」并给出两段范围，指向生成的表。同一张表确认 §5.1 的「M9b 八个重写语料在 v1 下作废七个」无误
（M9 + M9b 合计：v1 作废 9/16，v2 作废 2/16）。

## 2026-09-18（顾问第 11 轮）
1. **§5.3 的探索性标签放进作出主张的那一句里**，并点名两个预注册结果把它夹住：M7 H1（无参考时 8/12 坍缩）与
   M8 A1（撤掉一个亲属的参考，该亲属在 6–8/8 对里重新坍缩）。**我自己的句子也有同样的错**：我曾写
   「pooled rejector = 同一读出、只改校准」，但 T2g 用的是自己的「所有者 vs 其余四个教师合并」读出，与 T0 的多类读出不同。
   干净的单变量对比在 **T2g 内部**：同一检验、同一读出、同一校准，远亲 0.0、近亲 1.0，且只在 GSM8K。已改写 §5.3、
   §5.3 的反驳段、以及 draft_analysis 的证据条目，全部标注 exploratory、GSM8K。
2. **M13 的披露以算术开头**：「9 个校准分数 → 最小可达 p = 0.10，无论数据如何门槛都不可能通过」，然后才说何时发现
   （所有者自己的学生检出率 0.00 时）。修正依据只来自 n_cal，恢复的是 M7 预注册的校准规模而非新选的。
   **下限规则写进方法**（§5.3 T0 定义处）：共形/秩门槛需 n_cal ≥ 19 才能到 α = 0.05，作为可复用的教训推荐给读者。
3. **规模限制降级而非删除**：「在所测的那一格里不是小学生模型的伪影」，不是「与规模无关」；坍缩 7/12 对门槛 6 是险过，
   三分之一分辨率，一格、一数据集、一读出，学生仍是 LoRA，全参未测。并加一句：补救很强（11/12、0.03、0.94）而坍缩边缘——
   这是论文第三次看到「失败严重度变、修复不变」（读出/数据集、厂商、学生规模）。
4. **ARC（M3 G-R3）**：只放附录、两种读法、以预注册读法为判词（按运行时的长度检查为 void）。看到结果后采用的长度规则
   直接决定结论，不能产生通过。与 M13「事后数字不是结果」同一纪律。
5. **保留的实验继续保留**，现在就写两句 limitation（分类器感知攻击者：只会更强，故我们的攻击结果是下界；非数学任务：
   主张限定在数学推理轨迹）。已写入 §6.4 Limitations，另加规模与第一方访问模型两条。
6. **§6.4 防御指向 Krishna et al. (NeurIPS 2023)**：所有者**发布了**轨迹，因此恰好持有检索式检测所需的数据库。
   **同时修正我此前的一句**：原文写「内容成员检验败给转述」，这与 Krishna 的结论（检索能扛住转述，至少在文本层面）
   相矛盾。现改为：内容信号不受模仿影响；检索在文本层面扛住转述；两件事未测试——蒸馏后是否仍成立（学生输出是生成的
   而非复制的）、能否区分兄弟检查点（它们回答同一批题）；且它放弃了本文的留出探针设计。所以只作方向，不作结果。

## 2026-09-18 外部调研报告（后续方向 D1–D12）核查后对**当前论文**的更正
- **Rawat et al. (2607.09692) 我此前的描述是错的**（09-16 记为「需要教师早期检查点的权重」）。全文阅读后：参考 = **学生自己家族**
  的早期检查点；统计量 = 学生 NLL − 参考 NLL（在候选教师的输出上），需要学生与参考的 log-prob，教师黑盒；有二项/t 检验，
  阈值按 F1 在校准格上选；候选全是跨厂商教师，从未测同线兄弟检查点；未测改写攻击（列为开放问题）。其 leave-one-teacher-out
  在 4/6 与 1/6 格出现误检——正是我们的「未覆盖候选」失败。§2.1 已重写：共享「参考归一化」这一动作，差别在参考覆盖什么。
  报告称其为 T1 的「近孪生」是夸大，但我们的定位句必须更精确，已改。
- **Liu et al. (2512.20908) 的会议**：ML Anthology（二手索引）列为 ICLR 2026 主会；arXiv 无会议信息、PDF 页眉为 Preprint；
...
## 2026-09-19 顾问第 13 轮：D7、D1 设计与第二个 7B 格
- **D7**：朴素检索通道会重开覆盖失败（在已发布题目上，任何答对的学生都与所有者轨迹相似，兄弟检查点的学生更近）。
  设计改为**成对、逐实例、逐亲属**：d_x = sim(y_x, t^a_x) − sim(y_x, t^b_x)，参考分布来自 a、b 的参考学生——继承 T1 的
  逐亲属结构，覆盖要求被继承并明说。预测：未受攻击时内容通道弱于风格（M11 EMB 0.60–0.66）；模仿攻击下检索仍成立；
  价格曲线的攻击轴是改**步骤内容**的改写。记录于 `.pipeline/docs/followup_designs.md`（设计笔记，非预注册）。
- **D1**：截断不是长度控制。两个条件（长度分箱配对 ±10%；再去掉 think 标签/Step N/boxed/答案行后配对）；门槛相对于
  「仅长度」基线（未配对数据上训练）：配对条件下全读出 ≥ 基线 + 0.10 AUC 且过置换零分布 95 分位，4 个单元中 ≥ 3 个；
  能力门槛：准确率 ≤ base − 0.03（v2 抽取器）。所检验的是**非长度**风格。
- **第二个 7B 格**：现在预注册、审稿期间运行，选最可能不利的 MATH·Qwen-7B；与 M13 同设计，校准 10/教师（下限 0.032）；
  措辞规则：若 G1 失败，写「补救在两个 7B 格都成立；标准检验的坍缩在一格存在、另一格更弱」——这是「失败严重度变、修复不变」
  的第四例，不是退让。
- **规模句改为先讲补救**（TPR 0.94 / FPR 0.03），坍缩写作「存在但窄」。已改 §6.1、§6.4。
- **写 M14 预注册时发现 M13 的一处配方偏差（待核实）**：M7 训练用 `M3C_MAXLEN=4608`，M13 用 1024，而 M13 设计文档声称
  1024 就是 M7 配方。对 GSM8K 只有样本超过 1024 token 时才有影响；正在测量。MATH 轨迹更长，M14 必须用 4608。
