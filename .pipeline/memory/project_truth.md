...
- **当前：M3 = 论文方向**（被动 prompt-only 溯源）。**M3 v3 稳健性已预注册（c500e86）并在跑（作业 20044642→43→44）**：64 密钥池（α=0.05 ⇔ 秩≤3）、第二教师 Qwen2.5-7B、ARC-Challenge 非数学域、蒸馏方攻击（正确性过滤 / Qwen 改写 / 压缩成算式），G-R0 败则停。
