...
- **M3 通过（2026-09-11，本会话第一个正门槛）**：被动 prompt-only 签名会被蒸馏继承——学生自归因 0.75（随机 0.25），两种学生规模复现；长度对照与置换 null 均排除伪影；**严格 null（clean 学生 + base 模型取大）下 k1/k5 成立、k0 不可主张**。
- **M3 v2 全过（2026-09-12）**：16 密钥 × 2 学生家族，所有者侧检验 6/6 均 p=1/16（可达下限），经验 FPR 0.016；
  150 条轨迹即可、50% 混合数据仍可检出；主动水印（ACL-2026 式）与被动路线互补而非竞争。
- **更正（2026-09-12）**：v2 的「经验 FPR 0.016」几乎由秩构造决定（每个学生至多一个密钥能到 1/16，受训学生的这个位置被真密钥占掉），**不能当有效性证据**；且 v2 的 16 个密钥分三类手写，不可交换，v2 的 p 值只作描述。有效性来自「所有者密钥与诱饵密钥同分布随机抽取」，v3 用生成器 64 密钥池 + 逐密钥 hot-key 条件检查替代。
- **M3 v3 稳健性完成（2026-09-13）**：G-R0 12/16（恰好过）、G-R2 第二教师 16/16、G-R3 ARC 14/16（逐臂操作检查；初版合并读法 void，由用户决定报法）、**G-R1 改写败（8/16）**；filter 7/8、compress 3/8 预测成立；hot-key ≤0.047；10 次查询 0.79。
- **当前：M3 = 论文方向，主张收窄为「未改写 / 仅过滤的蒸馏」**，改写是声明的局限（与主动轨迹改写互补）。候选 v4：预注册检验「增加命题内容的锚点抗改写」。
