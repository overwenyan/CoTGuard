"""EXP-B1: 读出范式对比 —— 密钥指纹是语义的还是文体的?

本文主张把轨迹溯源当**文体归属**问题而非**语义匹配**问题. baseline 因此要横跨这两个
范式, 而不是去比那些机理上不适用的行为层水印(AgentMark/SeqWM 需要离散动作空间,
在纯文本 CoT 上无法运行 —— 那是 gap 论据, 不是可比 baseline).

注意 `trigger_sim` 在本文中是**"自然的语义匹配基线"**: 任何人拿到这个问题都会先想到
"看轨迹像不像 trigger 文本". 它值得测是因为它是最直觉的做法, 不是因为谁做过.

读出器, 按范式分组:
  length_only        仅轨迹步数与字符数        —— 平凡对照, 检验是否被长度平凡驱动
  [语义匹配范式]
  trigger_sim        与 trigger 文本的余弦相似度  —— 最直觉的做法
  embed_learned      句向量按步均值             —— 语义编码器 + 逻辑回归
  embed_meanstd      句向量 mean⊕std           —— 与整段词法特征信息量对等
  embed_doc          整段直接编码
  [文体归属范式]
  stylometry_strict  仅功能词频率 + 标点频率     —— **零内容信息**, 纯写作习惯
  stylometry         上者 + 句长/步长/字符类统计
  tfidf_word/char    词级 / 字符级 TF-IDF n-gram —— 廉价词法, 无语义模型
